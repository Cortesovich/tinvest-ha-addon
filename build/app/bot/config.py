...
class Config:
    ...
    # --- параметры (config.yaml) ---
        account_type: str = "iis"
        coupon_lookahead_days: int = 180
        maturity_horizon_days: int = 1825
        hide_zero_coupons: bool = True
        tax_refund_amounts: list = field(default_factory=list)
        timezone: str = "Europe/Moscow"
        poll_timeout: int = 30

    # --- отбор облигаций для реинвеста (Этап 3) ---
        bond_min_maturity_days: int = 183
        bond_max_maturity_days: int = 1825
        bond_top_n: int = 3
        reinvest_min_cash: float = 1000.0
        bond_max_screened: int = 80
        bond_whitelist: list = field(default_factory=lambda: list(DEFAULT_BOND_WHITELIST))

    # --- покупка по кнопке (Этап C) ---
        trade_enabled: bool = False
        reinvest_max_rub: float = 0.0

    # --- топ-листы и отбор акций ---
        topbonds_n: int = 5
        stock_top_n: int = 10
        stock_min_market_cap: float = 0.0
        stock_min_free_float: float = 0.0
        stock_min_pe: float = 1.0
        w_value: float = 22.5
    ...
    # --- второй пользователь: только чтение (без покупок) ---
        viewer_chat_ids: list = field(default_factory=list)

    # --- автопокупка облигаций (Этап D) ---
        autobuy_enabled: bool = False
        autobuy_min_cash: float = 1200.0
        autobuy_days: list = field(default_factory=lambda: ["mon", "fri"])
        autobuy_time: str = "11:00"
